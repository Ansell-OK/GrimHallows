...
import { createParty, createPartyInvite, errorMessage, getCharacters, getCurrentParty, getPartyInvites, leaveParty, respondPartyInvite, setPartyCharacter, setPartyReady, type PartyInvite, type PartyRecord } from '@/lib/api';
...
export default function Party() {
  ...
  const [characters, setCharacters] = useState<readonly { contractId: string; tokenId: string; name: string; charClass: string }[]>([]);
  const load = useCallback(async () => {
    if (!address) { setParty(null); setInvites([]); return; }
    try { const [current, pending, owned] = await Promise.all([getCurrentParty(), getPartyInvites(), getCharacters(address)]); setParty(current.party); setInvites(pending.invites); setCharacters(owned.characters); setError(null); }
